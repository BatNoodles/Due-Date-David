...
public class DueDateReminderHandler {
    public static void handleRemind(GatewayDiscordClient client){
        ...
        database.filterDueDates(d -> d.getTimeUntil().toHoursPart() == 1).forEach(dueDate ->
                client.getChannelById(Snowflake.of(445386053042307072L)) //Hardcoded id, will be changed later.
